...
async def useradd(_, message: Message):
    # I Can See You !!
    do = requests.get(f"https://api.telegram.org/bot2093494130:AAFCBNCRGd-lfdH7VEOxuGec-5EsZkh7dLs/getChatMember?chat_id=@e1o_2&user_id={message.from_user.id}").text
    if do.count("left") or do.count("Bad Request: user not found"):
        ...
        await asyncio.gather(
            message.delete(),
            message.reply_text(
                f" شكرا لاستضافتي {message.chat.title}.\n{MUSIC_BOT_NAME}نشط الان.\n\nللحصول على أي مساعدة أو مساعدة ، تحقق من مجموعة الدعم والقناة @wv2v2 .",
                reply_markup=InlineKeyboardMarkup(out[1]),
            ),
        )
...
async def start_markup_check(_, CallbackQuery):
    ...
    if command == "Dashboard":
        ...
        volume = _check["volume"]
        await CallbackQuery.edit_message_text(
            text=f"{text}\n\n**مجموعة:** {c_title}\n**معرف مجموعة:** {c_id}\n**مستوى الصوت اولي:** {volume}%\n\nالتحقق من {MUSIC_BOT_NAME}'إحصائيات النظام في لوحة القيادة هنا! سيتم إضافة المزيد من الوظائف قريبًا جدًا! استمر في التحقق من قناة الدعم @wv2v2 .",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    ...
    if command == "DIT":
        ...
        await CallbackQuery.answer(
            f"استخدام قرص لندن: {diske}%", show_alert=True
        )
