...
async def admin_risghts(_, CallbackQuery):
    ...
    if not await is_active_chat(CallbackQuery.message.chat.id):
        return await CallbackQuery.answer(
            "لا شيء يشغل في المكتامه الصوتية.", show_alert=True
        )
    ...
    if command == "pausecb":
        ...
        await CallbackQuery.message.reply_text(
            f"🎧  تم إيقافه  الصوت مؤقتًا بواسطه {CallbackQuery.from_user.mention}!",
            reply_markup=audio_markup2,
        )
        await CallbackQuery.message.delete()
        await CallbackQuery.answer("متوقف مؤقتًا", show_alert=True)
    if command == "resumecb":
        ...
        await CallbackQuery.message.reply_text(
            f"🎧 تم استئناف المحادثة الصوتية بواسطة {CallbackQuery.from_user.mention}!",
            reply_markup=audio_markup2,
        )
        await CallbackQuery.message.delete()
        await CallbackQuery.answer("مستأنف", show_alert=True)
    if command == "stopcb":
        ...
        await CallbackQuery.message.reply_text(
            f"🎧تم يقاف المحادثة الصوتية بواسطة {CallbackQuery.from_user.mention}!",
            reply_markup=audio_markup2,
        )
        await CallbackQuery.message.delete()
    ...
    if command == "skipcb":
        ...
        if Queues.is_empty(chat_id):
            ...
            await remove_active_video_chat(chat_id)
            await CallbackQuery.message.reply_text(
                f"لا يوجد موسيقى في قائمه التشغيل  \n\nتم مغادره المكالمه الجماعيه .. الزر المستخدم من قبل :- {CallbackQuery.from_user.mention}"
            )
            await stop_stream(chat_id)
            await CallbackQuery.message.delete()
            await CallbackQuery.answer(
            ...
            if str(finxx) == "s1s":
                ...
                mystic = await CallbackQuery.message.reply_text(
                    "تم تخطي! يرجى الانتظار تغيير بث الفيديو...."
                )
    ...
    if not _playlist:
        ...
        for shikhar in _playlist:
            ...
            if await is_active_chat(chat_id):
                ...
                for_p = 1
                msg += f"{j}- {title[:50]}\n"
                msg += f"موقف في قائمة الانتظار- {position}\n\n"
                ...
                cap = f"🎥<b>_يشغل:__ </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n💡<b>__معلومات:__</b> [احصل على معلومات إضافية](https://t.me/{BOT_USERNAME}?start=info_{videoid})\n👤**__بواسطه:__** {CallbackQuery.from_user.mention}"
                final_output = await CallbackQuery.message.reply_photo(
                    photo=thumb,
                    reply_markup=InlineKeyboardMarkup(buttons),
                    caption=cap,
        ...
        if for_p == 1:
            ...
            if await isPreviewUp(preview):
                await CallbackQuery.message.reply_photo(
                    photo=preview,
                    caption=f"هذه قائمة تشغيل من {third_name}.\n\nيشغل بواسطه :- {CallbackQuery.from_user.mention}",
                    quote=False,
                    reply_markup=InlineKeyboardMarkup(buttons),
                )
                await m.delete()
...
async def group_playlist(_, CallbackQuery):
    ...
    if _check:
        return await CallbackQuery.message.reply_text(
            f"{CallbackQuery.from_user.mention},إنه موجود بالفعل في قائمة التشغيل!"
        )
    assis = {
        "videoid": videoid,
        "title": title,
    ...
    return await CallbackQuery.message.reply_text(
        f"مضاف إلى {type}'s {genre} قائمة التشغيل بواسطة {CallbackQuery.from_user.mention}"
    )
...
async def check_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        return await CallbackQuery.answer(
            f"No {genre} قائمة التشغيل على الخوادم. حاول إضافة الموسيقى في قائمة التشغيل.",
            show_alert=True,
        )
    else:
        j = 0
        ...
        m = await CallbackQuery.message.reply_text("لصق قائمة التشغيل في سلة المهملات4")
        link = await paste_queue(msg)
        preview = link + "/preview.png"
        url = link + "/index.txt"
...
async def del_playlist(_, CallbackQuery):
    ...
    await CallbackQuery.message.reply_text(
        f"تم الحذف بنجاح {type}'s {genre} قائمة تشغيل كاملة\n\nBy :- {CallbackQuery.from_user.mention}"
    )
